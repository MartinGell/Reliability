...
new_reliability <- c(0.99)#c(0.99,0.95,0.9,0.85,0.8,0.75,0.7,0.65,0.6,0.55,0.5)   # desired correlation with empirical measure
...
maximum <- 160 # 4SD 
minimum <- 40  # < 30 is motor dysfunction (more than 4SD)

outdir <- '/home/mgell/Work/Prediction_HCP/text_files/rel/'
beh <- 'nih_totalcogcomp_ageadjusted' #'age_corrected_standard_score' #'nih_tlbx_agecsc_dominant' #'nih_crycogcomp_ageadjusted' nih_totalcogcomp_ageadjusted
plt_out <- '/home/mgell/Work/reliability/plots/'
fname <- 'beh_HCP_A_total' # HCP_A_motor HCP_A_lswm HCP_A_cryst beh_HCP_A_total
...
d = d[d[,beh] >= (mean(d[,beh]) - 3*sd(d[,beh])),]
df <- d
#d = d[d$nih_tlbx_agecsc_dominant < 145,]
#d = d[d$nih_tlbx_agecsc_dominant > 55,]
