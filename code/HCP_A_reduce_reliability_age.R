...
tab <- read.csv('/home/mgell/Work/Prediction_HCP/text_files/HCP_A_age.csv')
...
d = d[d[,beh] >= (mean(d[,beh]) - 3*sd(d[,beh])),]
# remove 100 year old subs outside of the sample distribution
d = d[d[,beh] < 1200,]
...
PDF = density(T1, bw = bw.SJ(T1), kernel = 'gaussian', from = min(T1), to = max(T1))

PDF_plt = PDF
PDF_plt$y = PDF_plt$y * 10000

png(file=paste0(plt_out,'hist_',beh,'.png'), width=400, height=400)
hist(d$interview_age,breaks = 40,xlim = c(300,1300),ylim = c(0,40))
...
for (rel_i in new_reliability) {
  ...
  for (i in 1:n) {
    ...
    beh_noise = data.frame('src_subject_id' = d$src_subject_id, 'interview_age' = T1_noisy_ok)
  ...
  hist(all_noisy[,eg1],breaks = 50,xlim = c(300,1300),ylim = c(0,40))
  dev.off()
  
  png(file=paste0(plt_out,'hist_example2_',beh,'_wnoise_rel_',rel_i,'.png'), width=400, height=400)
  hist(all_noisy[,eg2],breaks = 50,xlim = c(300,1300),ylim = c(0,40))
  dev.off()
  ...
  plt1 = ggplot(noisy_data, aes(x = Noisy_means)) + geom_histogram(col = "black", fill = "skyblue3", alpha=0.5, bins = 15) +
    ...
    coord_cartesian(xlim = c(empirical_mean - 50, empirical_mean + 50))
  ...
  plt2 = ggplot(noisy_data, aes(x = Noisy_sd)) + geom_histogram(col = "black", fill = "skyblue3", alpha=0.5, bins = 15) +
    ...
    coord_cartesian(xlim = c(empirical_sd - 20, empirical_sd + 20))
