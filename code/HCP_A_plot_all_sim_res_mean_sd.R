...
pltm = ggplot(df_sum, aes(x = reliability)) + 
  ...
  #scale_x_reverse()
ggsave(paste0(plt_out,'OHBM_All_variables_R2_mean_sd_wnoise_rel_',rel_max,'_to_',rel_min,'.png'), pltm, width=4, height=3)
...
# Fit exponential
model <- lm(log(d_age$test_R2)~d$reliability)
summary(model)
...
# colnames(df_sd)[2:11] <- metrics[c(-1,-2,-13,-14)]
# 
# df_m = df_m %>% select(c('reliability', 'test_RMSE'))
# df = cbind(df_m,df_sd$test_R2)
# colnames(df) = c('reliability', 'R2_mean', 'R2_sd')
# df$variable = 'age'
# 
...
# 
# df_sum = rbind(df_sum, df)
# df_sum$R2_mean = df_sum$R2_mean*-1
# 
# 
# # plot all together
# pltm = ggplot(df_sum, aes(x = reliability, fill = variable)) + 
#   geom_point(aes(y = R2_mean, colour = variable), size=1) +
#   geom_line(aes(y = R2_mean, colour = variable), size=0.8) +
#   geom_ribbon(aes(y = R2_mean, ymin = R2_mean - 2*R2_sd, ymax = R2_mean + 2*R2_sd), alpha = .2) +
#   theme_classic() + 
#   scale_y_continuous(limits = c(9,12),  name = 'MAE')
# ggsave(paste0(plt_out,'All_variables_MAE_mean_sd_wnoise_rel_',rel_max,'_to_',rel_min,'.png'), pltm, width=4.5, height=3.5)
