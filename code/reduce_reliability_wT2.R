...
# Load real data to crete T2 from and to add noise to
...
### CREATE T2 ###
# Create measurements at two timepoints with specific reliability
T1 <- d[,4]
# For T2 first create a random vector based on mean and sd of T1, then tweak it
# to have an exact correlation with T1
T2_temp <- rnorm(n = length(T1), mean = mean(T1), sd = sd(T1))
T2 <- noisier_beh(T1, true_reliability, T2_temp)
T2 <- min_max(T2, max = maximum)
...
for (rel_i in new_reliability) {
  ...
  for (i in 1:n) {
    ...
    T1_noisy <- noisier_beh(T2, rel_i, x)
    #plot(new/10,T1)
  ...
  print(cor(all_noisy[,1],T2))
  cor(all_noisy[,10],T2)
  cor(all_noisy[,33],T2)
  
  # correlation between T1_noisy and T1 is lower than specified correlation/reliability
  # between T1_noisy and T2 as there is less common variance.
  # If T1_noisy and T2 correlate at 0.85 and T1 and T2 correlate at 0.94, then
  # correlation between T1_noisy and T1 has to be 0.85*0.94:
  print('correlation of T1_noisy and T1')
  print(mean(all_cor))
  
  print('new reliability * true reliability')
  print(rel_i*true_reliability)
  
  write.csv(all_noisy, paste0(outdir,beh,'_wnoise_rel_wT2_',rel_i,'.csv'))
