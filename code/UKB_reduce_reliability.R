...
new_reliability <- c(0.7, 0.75) #c(0.99,0.95,0.9,0.85,0.8,0.75,0.7,0.65,0.6,0.55,0.5)    # desired correlation with empirical measure
n <- 100  # how many simulated noisy datasets to make

outdir <- '/home/mgell/Work/Prediction_HCP/text_files/rel/subsamples/'
beh <- 'Hand_grip_strength_mean_lr' #Hand_grip_strength_mean_lr  Age_when_attended_assessment_centre
...
for (rel_i in new_reliability) {
  ...
  # First run 100 times to get offset from sample mean.
  ...
  for (i in 1:1000) {
    ...
    T1_noisy_scaled <- round(T1_noisy/scaler, digits = 4) # Round as age is int
  ...
  for (i in 1:n) {
    ...
    T1_noisy_ok <- round((T1_noisy/scaler) + mean_offset, digits = 4)
    ...
    beh_noise = data.frame('src_subject_id' = d$eid, 'placeholder' = T1_noisy_ok)
    names(beh_noise)[names(beh_noise) == "placeholder"] <- paste0(beh,'-2.0')
  ...
  plt3 = ggplot(all_noisy_d,aes(x=Actual_data,y=Example_1_noisy_data)) + 
    ...
    annotate('text', x = 5.5, y = 60, label=paste0("r = ", round(cor(all_noisy_d$Actual_data, all_noisy_d$Example_1_noisy_data),digits = 2)), size=5)
  ...
  plt4 = ggplot(all_noisy_d,aes(x=Actual_data,y=Example_2_noisy_data)) + 
    geom_point(size=2.5, alpha=0.6, colour="skyblue3") + theme_classic() + 
    annotate('text', x = 5.5, y = 60, label=paste0("r = ", round(cor(all_noisy_d$Actual_data, all_noisy_d$Example_2_noisy_data),digits = 2)), size=5)
  ...
  write.csv(all_noisy, paste0('/home/mgell/Work/reliability/text_files/',beh,'_wnoise_rel_',rel_i,'.csv'))
