...
beh <- 'nih_crycogcomp_ageadjusted' #'age_corrected_standard_score' #'nih_tlbx_agecsc_dominant' #'nih_crycogcomp_ageadjusted'
plt_out <- '/home/mgell/Work/reliability/plots/'
fname <- 'HCP_A_cryst' # HCP_A_motor HCP_A_lswm HCP_A_cryst 
...
for (rel_i in new_reliability) {
  ...
  plt1 = ggplot(noisy_data, aes(x = Noisy_means)) + geom_histogram(col = "black", fill = "skyblue3", alpha=0.5, bins = 15) +
    ...
    theme_classic() + theme(legend.position="none") + ylab('count') + 
    coord_cartesian(xlim = c(empirical_mean - 20, empirical_mean + 20))
